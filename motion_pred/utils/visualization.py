...
def vis_asb(filepath1, filepath2, save_path, index):
    ...
    # edges = [(0, 1), (1, 2), (2, 3), (3, 4), (1, 5), (5, 6), (6, 7), (0, 8), (8, 9), (9, 10), (0, 11), (11, 12), (12, 13), (1, 14)]
    ...
    for i in range(index, index + 1):
        for j in range(0, all_gt.shape[1], 5):
            ...
            ax.grid(None)
            ax.axis('off')
            neck_x, neck_y, neck_z = all_gt[i, j, 0, 0], all_gt[i, j, 0, 1], all_gt[i, j, 0, 2]
            ax.scatter(neck_x, neck_y, neck_z, zdir="z", c='r', marker='o')
            gt_x = all_gt[i, j, 1:, 0]
            gt_y = all_gt[i, j, 1:, 1]
            gt_z = all_gt[i, j, 1:, 2]
            ax.scatter(gt_x, gt_y, gt_z, zdir="z", marker='o')
            for e in edges:
                ax.plot(all_gt[i, j, e, 0], all_gt[i, j, e, 1], all_gt[i, j, e, 2], c="#4882DA")

            pneck_x, pneck_y, pneck_z = all_pred1[i, j, 0, 0], all_pred1[i, j, 0, 1], all_pred1[i, j, 0, 2]
            ax.scatter(pneck_x, pneck_y, pneck_z, zdir="z", c='r', marker='^')
            pr_x = all_pred1[i, j, 1:, 0]
            pr_y = all_pred1[i, j, 1:, 1]
            pr_z = all_pred1[i, j, 1:, 2]
            ax.scatter(pr_x, pr_y, pr_z, zdir="z", marker='^')
            for e in edges:
                ax.plot(all_pred1[i, j, e, 0], all_pred1[i, j, e, 1], all_pred1[i, j, e, 2], c="#E99445")

            pneck_x2, pneck_y2, pneck_z2 = all_pred2[i, j, 0, 0], all_pred2[i, j, 0, 1], all_pred2[i, j, 0, 2]
            ax.scatter(pneck_x2, pneck_y2, pneck_z2, zdir="z", c='r', marker='s')
            pr_x2 = all_pred2[i, j, 1:, 0]
            pr_y2 = all_pred2[i, j, 1:, 1]
            pr_z2 = all_pred2[i, j, 1:, 2]
            ax.scatter(pr_x2, pr_y2, pr_z2, zdir="z", marker='s')
            for e in edges:
                ax.plot(all_pred2[i, j, e, 0], all_pred2[i, j, e, 1], all_pred2[i, j, e, 2], c="#11AF36")
        ...
        ax.grid(None)
        ax.axis('off')
        neck_x, neck_y, neck_z = all_gt[i, j_last, 0, 0], all_gt[i, j_last, 0, 1], all_gt[i, j_last, 0, 2]
        ax.scatter(neck_x, neck_y, neck_z, zdir="z", c='r', marker='o')
        gt_x = all_gt[i, j_last, 1:, 0]
        gt_y = all_gt[i, j_last, 1:, 1]
        gt_z = all_gt[i, j_last, 1:, 2]
        ax.scatter(gt_x, gt_y, gt_z, zdir="z", marker='o')
        for e in edges:
            ax.plot(all_gt[i, j_last, e, 0], all_gt[i, j_last, e, 1], all_gt[i, j_last, e, 2], c="#4882DA")

        pneck_x, pneck_y, pneck_z = all_pred1[i, j_last, 0, 0], all_pred1[i, j_last, 0, 1], all_pred1[i, j_last, 0, 2]
        ax.scatter(pneck_x, pneck_y, pneck_z, zdir="z", c='r', marker='^')
        pr_x = all_pred1[i, j_last, 1:, 0]
        pr_y = all_pred1[i, j_last, 1:, 1]
        pr_z = all_pred1[i, j_last, 1:, 2]
        ax.scatter(pr_x, pr_y, pr_z, zdir="z", marker='^')
        for e in edges:
            ax.plot(all_pred1[i, j_last, e, 0], all_pred1[i, j_last, e, 1], all_pred1[i, j_last, e, 2], c="#E99445")

        pneck_x2, pneck_y2, pneck_z2 = all_pred2[i, j_last, 0, 0], all_pred2[i, j_last, 0, 1], all_pred2[i, j_last, 0, 2]
        ax.scatter(pneck_x2, pneck_y2, pneck_z2, zdir="z", c='r', marker='s')
        pr_x2 = all_pred2[i, j_last, 1:, 0]
        pr_y2 = all_pred2[i, j_last, 1:, 1]
        pr_z2 = all_pred2[i, j_last, 1:, 2]
        ax.scatter(pr_x2, pr_y2, pr_z2, zdir="z", marker='s')
        for e in edges:
            ax.plot(all_pred2[i, j_last, e, 0], all_pred2[i, j_last, e, 1], all_pred2[i, j_last, e, 2], c="#11AF36")
