...
def get_scheduler(optimizer, policy, nepoch_fix=None, nepoch=None, decay_step=None, milestones=None, gamma=0.9):
    if policy == 'lambda':
        ...
        scheduler = lr_scheduler.StepLR( optimizer, step_size=decay_step, gamma=gamma)
    elif policy == 'plateau':
        scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.2, threshold=0.01, patience=5)
    elif policy == 'multistep':
        scheduler = lr_scheduler.MultiStepLR(optimizer, gamma=gamma, milestones=milestones)
    else:
