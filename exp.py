...
def random_mask_generator(batch_size, frame_num, joint_num, ratio):
    random_mask = torch.ones((batch_size, frame_num, joint_num, 1)).cuda().detach()
    random_mask = torch.dropout(random_mask, ratio, True)
    random_mask[random_mask > 0] = 1.0
    return random_mask
...
def train_asb(epoch, dct_m=None, idct_m=None, finetune=False):
    ...
    for traj_np in generator:
        ...
        mask = random_mask_generator(B, T, V, cfg.mask_ratio)
        traj_masked = traj * mask
        if cfg.dataset == 'asb':
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', default="h36m_25_100")
