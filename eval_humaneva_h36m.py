    ...
    for i, data in enumerate(data_gen):
        ...
        for algo in algos:
            ...
            if cfg.dataset == 'humaneva':
                pred = pred.reshape(pred.shape[0], cfg.t_pred, 14, 3)
                pred = post_process(pred, data[:, cfg.t_his:, :, :])
            elif cfg.dataset == 'h36m':
                pred = pred.reshape(pred.shape[0], cfg.t_pred, 16, 3)
                pred = post_process(pred, data[:, cfg.t_his:, :, :])
                
            all_gt.append(gt.reshape(1, cfg.t_pred, dataset.skeleton.num_joints(), 3)[0])
            all_pred.append(pred.reshape(cfg.t_pred, dataset.skeleton.num_joints(), 3))

            gt = gt.reshape(gt.shape[0], cfg.t_pred, dataset.skeleton.num_joints(), 3)

            for stats in stats_names:
                val = 0
                val += stats_func[stats](pred, gt) / num_seeds
...
def compute_extended_metrics():
    ...
    # ==================== FLOPs ====================
    logger.info('=' * 80)
    logger.info('3. FLOPs (Floating Point Operations)')
    logger.info('-' * 40)
    ...
    if cfg.dataset == 'humaneva':
        bone_pairs = [
            ...
            (5, 7), (6, 8), (1, 9), (1, 10), (9, 11),
            (10, 12), (11, 13), (12, 14)
        ]
        bone_pairs_adj = [(p-1, c-1) for p, c in bone_pairs[1:]]
    elif cfg.dataset == 'h36m':
        bone_pairs_h36m = [
        ...
        bone_pairs_adj = [(p-1, c-1) for p, c in bone_pairs_h36m[1:]]
    else:
        bone_pairs_adj = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', default='stats', choices=['stats', 'extended'])
